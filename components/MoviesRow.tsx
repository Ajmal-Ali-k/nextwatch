  ...
  return (
    <section className="relative mx-auto px-4 sm:px-6">
      ...
      <div className="flex items-start gap-0">
        ...
        <motion.div
          ref={scrollRef}
          className="scroll-linked-list flex min-w-0 flex-1 gap-3 sm:gap-5 overflow-x-scroll pb-4"
          style={{ maskImage }}
        >
          {movies.map((movie) => (
            <article
              key={movie.title}
              className="movie-card group flex shrink-0 flex-col"
            >
              <div className="relative mb-3 aspect-2/3 overflow-hidden rounded-sm shadow-none transition group-hover:shadow-[0_0_30px_rgba(214,33,42,0.6)]">
                <Image
                  src={movie.image}
                  alt={movie.title}
                  width={300}
              ...
              <p className="mt-1 text-xs sm:text-sm text-white/60">{movie.date}</p>
            </article>
          ))}
        </motion.div>
      </div>
