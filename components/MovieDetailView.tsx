...
function toRecommendedMovies(recommended: MovieDetailPageData["recommended"]): Movie[] {
...
export default function MovieDetailView({ data }: { data: MovieDetailPageData }) {
  const recommendedMovies = toRecommendedMovies(data.recommended);
        ...
        <div className="relative z-10 mx-auto flex min-h-screen container flex-col justify-center py-16">
          <div className="px-4 sm:px-6">
            <div className="mb-6">
              ...
              <p className="text-lg font-bold sm:text-xl">{data.releaseDateFormatted}</p>
              {data.runtimeLabel ? (
                <p className="mt-1 text-sm text-white/70">Runtime: {data.runtimeLabel}</p>
              ) : null}
            </div>
