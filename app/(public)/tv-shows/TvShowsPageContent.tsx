...
import {
  parseTvShowsDiscoverParams,
  useTvShowsDiscoverUrl,
} from "@/lib/tv/discoverUrlState";
import { cn } from "@/lib/utils";
import {
  TV_DISCOVER_SORT_OPTIONS,
  type TvDiscoverSortValue,
} from "@/lib/tmdb/tvDiscoverSort";
...
import type { NormalizedDiscoverTvShow } from "@/lib/tmdb/types";
...
export default function TvShowsPageContent() {
  ...
  const watchProvidersQuery = useQuery({
    queryKey: ["tv-watch-providers", watchRegion, "en-US"],
    queryFn: async ({ signal }) => {
      const params = new URLSearchParams({ watchRegion, language: "en-US" });
      const res = await fetch(`/api/tv/watch-providers?${params.toString()}`, {
        signal,
      });
  ...
  const discoverQuery = useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      const data = (await res.json()) as TvDiscoverJson & { error?: string };
  ...
  const shows = discoverQuery.data?.results ?? [];
    ...
    <main className="relative min-h-screen pb-16 pt-12 text-white">
      ...
      <div className="mx-auto container">
          ...
          <div className="max-w-3xl">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              <div className="relative">
                <label htmlFor="tv-sort" className="sr-only">
                  Sort by
                </label>
                <select
                  id="tv-sort"
                  value={sortBy}
                  onChange={(e) =>
                    replace({ sortBy: e.target.value as TvDiscoverSortValue, page: 1 })
                  }
              ...
              <div className="relative min-w-34">
                <label htmlFor="tv-genre" className="sr-only">
                  Genre
                </label>
                <select
                  id="tv-genre"
        ...
        <section className="mt-10 px-4 sm:px-6 lg:px-0">
            ...
            <div className="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6">
              {shows.map((m) => (
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-white/60">{formatReleaseDate(m.firstAirDate)}</p>
                  </article>
                </Link>
              ))}
            </div>
          ...
          {!platformUnavailable && !loading && !errorMessage && shows.length > 0 ? (
            <nav
          ...
          {!loading && !errorMessage && !platformUnavailable && shows.length === 0 ? (
            <p className="text-sm text-white/60">No TV shows found for these filters.</p>
          ) : null}
        </section>
      </div>
    </main>
