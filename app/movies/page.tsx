...
export default function NewMoviesPage() {
  ...
  const [sortBy, setSortBy] = useState<DiscoverSortValue>("primary_release_date.desc");
  ...
  useEffect(() => {
    queueMicrotask(() => {
      setPlatformSelection((s) => {
        if (s.kind === "custom") return { kind: "all" };
        if (
          s.kind === "preset" &&
          isIndiaOnlyOttKey(s.key) &&
  ...
  const platformFilterLabel = useMemo(() => {
    if (platformSelection.kind === "all") return "all";
    if (platformSelection.kind === "preset") return platformSelection.key;
    return `custom:${platformSelection.providerId}`;
  }, [platformSelection]);

  const filterBaseKey = useMemo(
    () =>
      `${watchRegion}:${language}:${platformFilterLabel}:${sortBy}:${genreId ?? "all"}`,
    [watchRegion, language, platformFilterLabel, sortBy, genreId]
  );
  const filterBaseRef = useRef<string | null>(null);

  const platformUnavailable =
    platformSelection.kind === "preset" &&
    getProviderIdForRegion(watchRegion, platformSelection.key) === undefined;

  const indiaOnlyPlatformUnavailable =
    platformUnavailable &&
    platformSelection.kind === "preset" &&
    isIndiaOnlyOttKey(platformSelection.key);

  useEffect(() => {
    if (platformUnavailable) {
      filterBaseRef.current = filterBaseKey;
      queueMicrotask(() => setPage(1));
      return;
    }
    if (filterBaseRef.current !== null && filterBaseRef.current !== filterBaseKey) {
      queueMicrotask(() => setPage(1));
    }
    filterBaseRef.current = filterBaseKey;
  }, [filterBaseKey, platformUnavailable]);
  ...
  }, [discoverEnabled, discoverQuery.data, page, platformUnavailable]);
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              <div className="relative">
                ...
                <select
                  id="movies-sort"
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value as DiscoverSortValue)}
