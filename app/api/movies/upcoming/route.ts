...
function buildUpcomingUrl(apiKey: string, region: string, tmdbPage: number): string {
  const url = new URL(`${TMDB_API_V3_BASE}/movie/upcoming`);
  url.searchParams.set("api_key", apiKey);
  url.searchParams.set("language", UPCOMING_RESPONSE_LANGUAGE);
...
export async function GET(request: Request) {
  ...
  while (merged.length < RESULTS_PER_VIEW && tmdbPage <= TMDB_MAX_PAGE) {
    const url = buildUpcomingUrl(apiKey, watchRegionRaw, tmdbPage);
