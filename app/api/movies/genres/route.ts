...
async function fetchTmdbGenreList(
  apiKey: string,
  language: string
): Promise<{ genres: { id: number; name: string }[]; ok: boolean }> {
  const url = new URL(`${TMDB_API_V3_BASE}/genre/movie/list`);
...
export async function GET(request: Request) {
  ...
  let { genres } = await fetchTmdbGenreList(apiKey, language);
  ...
  if (
    ...
    genres = (await fetchTmdbGenreList(apiKey, primary)).genres;
  }

  if (genres.length === 0 && language !== "en-US") {
    genres = (await fetchTmdbGenreList(apiKey, "en-US")).genres;
  }
  ...
  if (genres.some((g) => !String(g.name ?? "").trim())) {
    const { genres: enGenres } = await fetchTmdbGenreList(apiKey, "en-US");
